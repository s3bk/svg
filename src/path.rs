...
use roxmltree::{Node};
use crate::{Error, Item, ItemKind, DebugInfo, parse_attrs, vec};
...
pub fn parse_path(node: &Node) -> Result<Item, Error> {
                ...
                PathSegment::EllipticalArc { abs, rx, ry, x_axis_rotation, large_arc, sweep, x, y } => {
                    ...
                    if r.x().is_finite() & r.y().is_finite() {
                        ...
                        let (a, b, c) = if gamma <= 1.0 {
                            ...
                            // c'
                            let s = vec(1., -1.) * r * (q * r_inv).yx() * dbg!(safe_sqrt((r2_prod - rq2_sum) / rq2_sum)) * dbg!(sign);
                            dbg!(s);
                            print_matrix(rot);
